...
| Thời gian reflection | 2026-05-10 đến 2026-08-02 |
...
```text
Trong quá trình phát triển LuxeWay từ lúc bắt đầu project đến ngày 2026-08-02, em sử dụng
...
Sau ngày 2026-06-28, em tiếp tục sử dụng AI để rà soát các lỗi phát sinh sau merge và sau khi bổ sung contract/payment. Các phần đáng chú ý gồm: phân tích xung đột giữa booking legacy và booking unified, sửa Google OAuth runtime do backend chưa load `.env`, tối ưu trang map và geolocation, phát triển dashboard/sidebar summary cho customer/owner/admin, kiểm tra mục Reviews và bổ sung dữ liệu unified review đúng contract.
...
- [x] Tối ưu map/geolocation và trải nghiệm chọn điểm
- [x] Phát triển dashboard sidebar summary theo vai trò
- [x] Kiểm tra Reviews, rating stats và dữ liệu seed unified
- [x] Sửa lỗi OAuth runtime do cấu hình môi trường
...
Sau đó, AI tiếp tục giúp em nhìn rõ hơn các lỗi không nằm ở một file đơn lẻ. Ví dụ lỗi Google Sign-In
không phải do frontend gửi sai token, mà do backend runtime không load đúng biến môi trường nên so sánh
audience với default client ID. Lỗi Reviews không phải do component React không render, mà do backend/API
trả dữ liệu rỗng vì seed legacy chưa tạo unified `vehicles + bookings + reviews`. Với dashboard, AI giúp em
chuyển từ cách frontend tự suy đoán số liệu sang API summary rõ ràng cho từng vai trò.
```
...
Ở giai đoạn sau 28/06, em học thêm rằng cấu hình runtime cũng quan trọng như code. Một chức năng có thể
compile/build thành công nhưng vẫn lỗi khi chạy nếu biến môi trường, profile H2, OAuth client ID hoặc dữ liệu
seed không đồng bộ. Em cũng hiểu rõ hơn lợi ích của việc tạo endpoint tổng hợp cho dashboard thay vì để UI
gom nhiều nguồn dữ liệu rời rạc.
```
...
Với các lỗi sau merge như Reviews hoặc OAuth, AI có thể ban đầu nghi ngờ UI hoặc frontend service, nhưng em
phải kiểm tra thêm API response, database và log runtime. Điều này giúp tránh sửa nhầm giao diện trong khi
nguyên nhân thật nằm ở seed data hoặc cấu hình backend.
```
...
Với Google OAuth, em kiểm tra từ Google client ID trong frontend `.env.production`, backend `.env`,
`application.yml`, profile H2 và log backend khi verify token. Với Reviews, em kiểm tra từ ReviewsPage,
reviewService, ReviewController, ReviewService, ReviewRepository đến dữ liệu trong H2. Với dashboard summary,
em kiểm tra API customer/owner/admin summary trả đúng mã trạng thái và frontend dashboard nhận đúng dữ liệu.
```
...
Một ví dụ sau khi kiểm tra Reviews:

| Nội dung | Mô tả |
|---|---|
| Gợi ý chưa đủ | Chỉ sửa empty state của ReviewsPage vì trang đang hiện 0 review |
| Rủi ro | UI đẹp hơn nhưng backend vẫn không có dữ liệu review, stats vẫn 0.0 |
| Cách cải thiện | Kiểm tra API `/reviews`, `/reviews/stats`, repository và seeder; bổ sung unified review seed có điều kiện |

Một ví dụ với OAuth:

| Nội dung | Mô tả |
|---|---|
| Gợi ý chưa đủ | Chỉ thay Google client ID ở frontend |
| Rủi ro | Token vẫn bị backend từ chối vì backend runtime đang dùng default client ID |
| Cách cải thiện | Kiểm tra log backend, profile H2 và thêm `spring.config.import` để load `.env` tự động |
...
Sau các lần sửa tiếp theo, em cũng trực tiếp xác định phạm vi cần kiểm tra khi một chức năng lỗi trên web:
không chỉ nhìn screenshot mà phải truy từ route frontend -> service -> API -> controller -> service backend
-> repository/database -> cấu hình runtime. Những phần em đóng góp thêm gồm tối ưu map UX, thiết kế API
dashboard sidebar summary, kiểm tra Reviews module và phân biệt lỗi dữ liệu với lỗi giao diện.
```
...
| Map | Panel xe có thể che bản đồ, geolocation có thể timeout | Có hướng tối ưu bố cục và fallback chọn điểm thủ công | Dễ dùng hơn |
| Dashboard sidebar | Sidebar chủ yếu tĩnh hoặc suy đoán từ frontend | Có API summary theo customer/owner/admin | Dữ liệu rõ hơn |
| Reviews | API trả rỗng vì seed legacy không tạo unified reviews | Có seed unified review, stats và empty state đúng hơn | Trang review có dữ liệu thật hơn |
| OAuth runtime | File env đúng nhưng runtime có thể không load | Backend tự import `.env` khi chạy local/H2 | Ít lỗi cấu hình hơn |
...
Các lỗi giai đoạn sau giúp em hiểu thêm rằng dữ liệu seed và cấu hình môi trường là một phần của chất lượng
phần mềm. Nếu database demo không phản ánh đúng domain unified, các trang như Reviews hoặc Dashboard sẽ
hiển thị sai dù code frontend không lỗi. Nếu runtime không load đúng `.env`, OAuth/payment có thể lỗi dù
source code nhìn đúng.
```
