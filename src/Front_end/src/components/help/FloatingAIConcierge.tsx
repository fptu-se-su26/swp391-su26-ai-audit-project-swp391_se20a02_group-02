...
export const FloatingAIConcierge: React.FC = () => {
  ...
  const renderActionCard = (msg: Message) => {
    ...
    if (msg.actionCard.action === 'VEHICLE_SEARCH' && msg.actionCard.vehicles) {
      return (
        <div className="mt-3 flex flex-col gap-2 w-full">
          <div className="flex items-center justify-between text-xs text-amber-300 font-semibold px-1">
            <span>{msg.actionCard.message || 'Matching Luxury Vehicles'}</span>
          </div>
          <div className="grid grid-cols-1 gap-2.5">
            {msg.actionCard.vehicles.map((v: any) => (
              <div
                key={v.id}
                onClick={() => navigate(`/vehicles/${v.id}`)}
                className="group flex items-center gap-3 rounded-lg border border-slate-800 bg-[#111B2E] p-2.5 transition duration-200 hover:border-[#D4AF37]/50 hover:bg-[#16223B] cursor-pointer"
              >
                <img
                  src={resolveImageUrl(v.thumbnailUrl || '/images/cars/placeholder.jpg')}
                  alt={v.name}
                  className="h-14 w-20 object-cover rounded-md border border-slate-700/50"
                  onError={(e) => { (e.target as HTMLImageElement).src = 'https://images.unsplash.com/photo-1549399542-7e3f8b79c341?auto=format&fit=crop&w=300&q=80'; }}
                />
                <div className="flex-1 min-w-0">
                  <h4 className="truncate text-xs font-semibold text-white group-hover:text-[#D4AF37] transition-colors">{v.name}</h4>
                  <div className="flex items-center gap-2 text-[11px] text-slate-400 mt-0.5">
                    <span>📍 {v.city || 'Việt Nam'}</span>
                    <span>•</span>
                    <span className="text-amber-300 font-medium">⭐ {v.rating || 5.0}</span>
                  </div>
                  <div className="text-xs font-bold text-[#D4AF37] mt-1">
                    {formatCurrency(v.pricePerDay || 0, currency)}<span className="text-[10px] font-normal text-slate-400">/day</span>
                  </div>
                </div>
                <div className="shrink-0 text-xs font-semibold text-[#D4AF37] opacity-80 group-hover:opacity-100 flex items-center gap-1">
