...
export function getStatusColor(status: string): string {
  const colors: Record<string, string> = {
    ...
    completed: 'bg-slate-50 dark:bg-slate-950 text-slate-700 dark:text-slate-300 border-slate-200 dark:border-slate-700',
  ...
  return colors[status] || 'bg-slate-50 dark:bg-slate-950 text-slate-700 dark:text-slate-300 border-slate-200 dark:border-slate-700';
}
