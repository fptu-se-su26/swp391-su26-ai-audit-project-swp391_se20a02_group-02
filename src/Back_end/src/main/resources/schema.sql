...
-- ============================================================
-- 36. API_PROVIDERS (External API Configuration Management)
-- ============================================================
IF NOT EXISTS (SELECT 1 FROM sys.tables WHERE name = 'api_providers')
BEGIN
...
CREATE INDEX IDX_api_providers_name         ON api_providers(provider_name);
...
-- Sample KYC Providers
IF NOT EXISTS (SELECT 1 FROM api_providers WHERE provider_name = 'ViaSoft KYC')
BEGIN
    INSERT INTO api_providers (id, provider_name, provider_type, base_url, api_key, secret_key, is_active, is_primary, description, retry_attempts, timeout_seconds)
    VALUES 
    (NEWID(), 'ViaSoft KYC', 'KYC', 'https://api.viasoft.vn/kyc', 'viasoft_api_key_here', 'viasoft_secret_key_here', 1, 1, 'ViaSoft KYC service for identity verification', 3, 30),
    (NEWID(), 'Face++ KYC', 'KYC', 'https://api.megvii.com/faceid/v3', 'faceplus_api_key_here', 'faceplus_secret_key_here', 1, 0, 'Megvii Face++ for facial recognition', 3, 30);
END
GO
...
BEGIN
    INSERT INTO api_providers (id, provider_name, provider_type, base_url, api_key, secret_key, is_active, is_primary, description, webhook_url, retry_attempts, timeout_seconds)
    VALUES 
    (NEWID(), 'Stripe', 'PAYMENT', 'https://api.stripe.com', 'pk_test_stripe_key_here', 'sk_test_stripe_secret_here', 1, 1, 'Stripe payment gateway', 'https://yourdomain.com/webhooks/stripe', 3, 30),
    (NEWID(), 'PayPal', 'PAYMENT', 'https://api.sandbox.paypal.com', 'paypal_client_id_here', 'paypal_client_secret_here', 1, 0, 'PayPal payment gateway', 'https://yourdomain.com/webhooks/paypal', 3, 30);
END
GO

-- Sample Email/SMS Providers
IF NOT EXISTS (SELECT 1 FROM api_providers WHERE provider_name = 'SendGrid')
BEGIN
    INSERT INTO api_providers (id, provider_name, provider_type, base_url, api_key, is_active, is_primary, description, retry_attempts, timeout_seconds)
    VALUES 
    (NEWID(), 'SendGrid', 'EMAIL', 'https://api.sendgrid.com', 'sendgrid_api_key_here', 1, 1, 'SendGrid email service', 3, 30),
    (NEWID(), 'Twilio', 'SMS', 'https://api.twilio.com', 'twilio_account_sid_here', 1, 1, 'Twilio SMS service', 3, 30);
END
GO

PRINT 'LuxeWay - All 36 tables created successfully on SQL Server (including API_PROVIDERS).';
GO