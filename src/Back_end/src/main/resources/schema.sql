...
CREATE TABLE user_documents (
    ...
    document_type   NVARCHAR(50)    NOT NULL CONSTRAINT CHK_user_docs_type CHECK (document_type IN ('PASSPORT','NATIONAL_ID','DRIVING_LICENSE','INSURANCE','CCCD_FRONT','CCCD_BACK','DRIVER_LICENSE_FRONT','DRIVER_LICENSE_BACK','MOTORBIKE_LICENSE_FRONT','MOTORBIKE_LICENSE_BACK','SELFIE','SELFIE_WITH_ID','INSURANCE_CERTIFICATE')),
...
CREATE TABLE bookings (
    ...
    owner_id            NVARCHAR(36)    NOT NULL,
    status              NVARCHAR(32)    NOT NULL DEFAULT 'PENDING' CONSTRAINT CHK_bookings_status CHECK (status IN ('PENDING','CONFIRMED','PICKING_UP','IN_PROGRESS','ACTIVE','DISPUTED','DRAFT','WAITING_PAYMENT','PAYMENT_PENDING','PAYMENT_VERIFIED','OWNER_APPROVED','READY_FOR_PICKUP','CHECKED_OUT','IN_RENTAL','RETURN_PENDING','RETURN_COMPLETED','COMPLETED','PAYMENT_EXPIRED','PAYMENT_REJECTED','CANCELLATION_REQUESTED','CUSTOMER_CANCELLED','OWNER_CANCELLED','SYSTEM_CANCELLED')),
