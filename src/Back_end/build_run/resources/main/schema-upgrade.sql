...
BEGIN
    CREATE TABLE vehicle_pricing_rules (
        id VARCHAR(36) PRIMARY KEY,
        vehicle_id VARCHAR(36) NOT NULL,
        rule_type VARCHAR(20) NOT NULL CHECK (rule_type IN ('WEEKEND', 'HOLIDAY', 'SEASONAL')),
...
BEGIN
    CREATE TABLE booking_delivery (
        ...
        latitude DECIMAL(10,8) NOT NULL,
        longitude DECIMAL(11,8) NOT NULL,
        status VARCHAR(30) NOT NULL CHECK (status IN ('PENDING', 'IN_TRANSIT', 'DELIVERED', 'RETURNED')),
